...
test('should getAccount (preview) with a HttpProvider', async () => {
    const provider = createHttpProvider(ChainIds.Mainnet);
    const address = "0x19dAe549081a1D5881eE925889685846fE06Fd9f";
    const account = await provider.getAccount(address, true);
    console.log('account == ');
    console.log(account);
...
test('should getTransactionCount (preview) with a HttpProvider', async () => {
    const provider = createHttpProvider(ChainIds.Mainnet);
    const address = "0x19dAe549081a1D5881eE925889685846fE06Fd9f";
    const count = await provider.getTransactionCount(address, true);
    ...
    request:
        {
            ...
            url: 'https://dnero-bridge-rpc.dnerochain.xyz/rpc'
        },
    response:
        {
            url: 'https://dnero-bridge-rpc.dnerochain.xyz/rpc',
